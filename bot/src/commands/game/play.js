...
const napi_rs_1 = require("canvas-constructor/napi-rs");
const MessageComponentCollector_1 = require("../../structures/MessageComponentCollector");
...
class TermoCommand extends BaseCommand_1.BaseCommand {
    constructor(client) {
        super(client, {
            name: 'jogar',
    ...
    async execute(ctx) {
        try {
            ...
            let game = this.client.games.find((n) => n.userid == ctx.author?.id);
            if (game) {
            ...
            let mode = ctx.options?.find((n) => n.name === 'modo')?.value || '5';
            let Words = await fs_1.promises.readFile(`${process.cwd()}/src/words/pt-br/${mode}_letras.txt`);
            let AllWords = await fs_1.promises.readFile(`${process.cwd()}/src/words/pt-br/dicio.txt`);
            let file = await fs_1.promises.readFile(`${process.cwd()}/src/images/tablets/tablet_${mode}_letters.png`);
            if (file) {
                ...
                let CMPS = new Components_1.Components(new Components_1.Button('Responder', 3, { custom_id: 'answer' }), new Components_1.Button('Desistir', 4, { custom_id: 'giveup' }), new Components_1.Button('Jogue Termo no seu próprio servidor!', 5, { url: `https://discord.com/oauth2/authorize?client_id=${this.client.user.id}&permissions=137506417664&scope=applications.commands%20bot` }));
                let ended = false;
                let words = [];
                let status = '';
                let tries = 0;
                ...
                let collector = new MessageComponentCollector_1.MessageComponentCollector(this.client, {
                    timeout: 1000 * 60 * 14,
                    filter: (message, author) => message.id == msg.id && author.id === ctx.author?.id
                });
                        ...
                        if (cmp) {
                            if (cmp.data?.custom_id === 'answer') {
                                if (cmp.author?.id === ctx.author?.id) {
                                    ...
                                    let msgc = new MessageCollector_1.MessageCollector(this.client, {
                                        channel_id: ctx.channel?.id || '',
                                        timeout: 1000 * 60,
                                        filter: (channel, author) => author.id == ctx.author?.id,
                                        max: 1
                                    });
                                    msgc.on('collect', async (msg) => {
                                        try {
                ...
                collector.on('end', async () => {
                    try {
                        let final_embed = new Embed_1.Embed()
                            //@ts-ignore
                        ...
                        if (status === 'lost' || status == 'giveup') {
                            ...
                            if (status == 'giveup')
                                embed.setImage(`attachment://tablet.png`);
                        ...
                        let gamei = this.client.games.findIndex((n) => n.userid == ctx.author?.id);
                        if (gamei != -1) {
    ...
    async generateTablet(tabletFile, mode, word, keys = ['']) {
        let img = await (0, napi_rs_1.loadImage)(tabletFile);
        let canvas = new napi_rs_1.Canvas(812, 985).printImage(img, 0, 0, 812, 985);
        for (let y = 0; y < keys.length; y++) {
            ...
            for (let i = 0; i <= Number(mode); i++) {
                let l_pos_x = 0;
                if (mode === '5')
                    l_pos_x = i == 0 ? 50 : i == 1 ? 210 : i == 2 ? 370 : i == 3 ? 530 : i == 4 ? 690 : 50;
                else
                    l_pos_x = i == 0 ? 68 : i == 1 ? 270 : i == 2 ? 470 : i == 3 ? 670 : 3000;
                let b_pos_x = 0;
                if (mode === '5')
                    b_pos_x = i == 0 ? 6 : i == 1 ? 167 : i == 2 ? 327 : i == 3 ? 487 : i == 4 ? 647 : 50;
                else
                    b_pos_x = i == 0 ? 8 : i == 1 ? 210 : i == 2 ? 411 : i == 3 ? 611 : 3000;
                ...
                let b = order[i]?.bg;
                if (l && b) {
                    let Bg = await fs_1.promises.readFile(`${process.cwd()}/src/images/bgs/${b}.png`);
                    let bg = await (0, napi_rs_1.loadImage)(Bg);
                    canvas.printImage(bg, b_pos_x, b_pos_y, (mode == '4' ? 192 : 156), 156);
                    let Letter = await fs_1.promises.readFile(`${process.cwd()}/src/images/letters/letter_${l}.png`);
                    let letter = await (0, napi_rs_1.loadImage)(Letter);

...
class FeedbackCommand extends BaseCommand_1.BaseCommand {
    constructor(client) {
        super(client, {
            name: 'feedback',
    ...
    async execute(ctx) {
        ...
        let CMPS = new Components_1.Components(new Components_1.SelectMenu('type', [{
                label: 'Avaliação',
        ...
        let col = new MessageComponentCollector_1.MessageComponentCollector(this.client, {
            timeout: 1000 * 60 * 5,
            filter: (message, user, id) => message.id == msg.id && user?.id === ctx.author?.id && id === 'type'
        });
        let col2;
        col.on('collect', async (select) => {
            try {
                ...
                let components = [new Components_1.Components(new Modal_1.ModalTextInput('first', (value == 1 ? 1 : 2), (value == 1 ? 'Envie uma nota de 0 a 5' : value == 2 ? 'Descreva a sua sugestão de forma detalhada' : 'Descreva o bug de forma detalhada'), { placeholder: (value == 1 ? '5' : value == 2 ? 'Adicionar um comando que faça isso e isso...' : 'Eu achei um bug que quando você executa o comando...'), required: true, min_length: (value == 1 ? 1 : value == 2 ? 10 : 10), max_length: (value == 1 ? 1 : value == 2 ? 1000 : 4000) })).toJson];
                if (value == 1) {
                    components.push(new Components_1.Components(new Modal_1.ModalTextInput('second', 2, 'Envie uma avaliação sobre o bot', { placeholder: 'O bot é muito bom...', required: true, min_length: 10, max_length: 1000 })).toJson);
                }
                else if (value == 3) {
                    components.push(new Components_1.Components(new Modal_1.ModalTextInput('second', 1, 'Envie o link de um print/video do bug', { placeholder: 'https://prnt.sc/flTssHvSqvGK', required: true, max_length: 200 })).toJson);
                }
                let modal_res = new Modal_1.Modal(title, (value == '1' ? 'feedback' : value == 2 ? 'suggestion' : 'bug'), ...components);
                await select.sendModal(modal_res);
                col2 = new MessageComponentCollector_1.MessageComponentCollector(this.client, {
                    timeout: 1000 * 60 * 5,
                    filter: (message, user, id) => message.id == msg.id && user.id == ctx.author?.id && (id == 'feedback' || id == 'suggestion' || id == 'bug'),
                    max: 1
                });
                col2.on('collect', async (modal) => {
                    try {
                        await modal.reply({ type: '6 ' });
                        let data = modal.data;
                        if (data) {
                            if (data.custom_id == 'feedback') {
                                let rating = data.components.find((n) => n.components.find((n) => n.custom_id == 'first'))?.components?.[0].value;
                                if (isNaN(rating)) {
                                ...
                                let comment = data.components.find((n) => n.components.find((n) => n.custom_id == 'second'))?.components?.[0].value;
                                if (!rating || !comment) {
                                    throw Error('Missing rating or content');
                                }
                                let guild = this.client.guilds.get(this.client.config.SafeEnv.$BASE_GUILD_ID);
                                if (guild) {
                                    let channel = guild.channels.get(this.client.config.channels.rating);
                                    if (channel) {
                                ...
                                let guild = this.client.guilds.get(this.client.config.SafeEnv.$BASE_GUILD_ID);
                                if (guild) {
                                    let channel = guild.channels.get(this.client.config.channels.suggestions);
                                    if (channel) {
                                        let embed = new Embed_1.Embed()
                                            .setAuthor(modal.author?.username || 'Desconhecido', modal.author?.dynamicAvatarURL())
                                            .setColor(this.client.config.colors.embed_color)
                            ...
                            if (data.custom_id == 'bug') {
                                let content = data.components.find((n) => n.components.find((n) => n.custom_id == 'first'))?.components?.[0]?.value;
                                let link = data.components.find((n) => n.components.find((n) => n.custom_id == 'second'))?.components?.[0]?.value;
                                if (!content || !link) {
                                ...
                                let guild = this.client.guilds.get(this.client.config.SafeEnv.$BASE_GUILD_ID);
                                if (guild) {
                                    let channel = guild.channels.get(this.client.config.channels.bugs);
                                    if (channel) {
                                        let embed = new Embed_1.Embed()
                                            .setAuthor(modal.author?.username || 'Desconhecido', modal.author?.dynamicAvatarURL())
                                            .setColor(this.client.config.colors.embed_color)
